...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		...
		Label labelMaxResult =new Label("MaxResult:");
		...
		Label labelCoinsResultP1=new Label("Coins of P1:");
		...
		Label labelCoinsResultP2=new Label("Coins of P2:");
		...
		Result.setTextFill(Color.CYAN);
		Result.setBackground(new Background(new BackgroundFill(Color.PURPLE, new CornerRadii(90), Insets.EMPTY)));
		Result.setPrefSize(70,40);
		Result.setTranslateX(-200);
		
		Button Animation=new Button("Animation");
		Animation.setTextFill(Color.CYAN);
		Animation.setBackground(new Background(new BackgroundFill(Color.PURPLE, new CornerRadii(90), Insets.EMPTY)));
		Animation.setPrefSize(100,40);
		Animation.setTranslateX(-20);
		
		
		Button Clear=new Button("Clear");
		Clear.setTextFill(Color.CYAN);
		Clear.setBackground(new Background(new BackgroundFill(Color.RED, new CornerRadii(90), Insets.EMPTY)));
		Clear.setPrefSize(70,40);
		Clear.setTranslateX(-220);
        ...
        //Check if the number of coins is odd, execute the condition sentence, and if it is even, execute the else
        if(n%2!=0){
			...
			for (int i = n - 1; i >= 0; i--) { //row
				for (int j = i; j < n; j++) {  //colume
					if (i == j) {
						...
						// If i or j are outside the bounds of the Index, then run the following condition
						if ((i + 1 | i + 2) > n || (j - 1) < 0) {
							op1 = array[i];
						} else {
			...
			txtArea.setText(stt);
				...
				txtnote.setText("enter all inde correct :)");
		...
		root.addRow(4, lablenote, txtnote);
